...
app.post('/deploy', async (req, res) => {
  ...
  try {
    ...
    await axios.patch(
      `https://api.heroku.com/apps/${realAppName}/config-vars`,
    ...
    await axios.post(
      `https://api.heroku.com/apps/${realAppName}/builds`,
      {
        source_blob: {
    ...
    // Step 4: Get real web_url
    const appInfoRes = await axios.get(
      `https://api.heroku.com/apps/${realAppName}`,
      { headers: herokuHeaders }
    );
    const realWebUrl = appInfoRes.data.web_url;

    // ✅ Send exact web_url to user
    res.send(`✅ Deployment started: ${realWebUrl}`);
  } catch (error) {
